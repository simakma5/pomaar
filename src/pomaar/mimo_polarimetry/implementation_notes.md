...
* **Key Implementation Details and Workarounds:**
  ...
  * **Relative Coordinate System Stripping:** When template geometries are copied, HFSS automatically carries over their parent relative coordinate systems (e.g., `RelativeCS1`, `PhaseCentreCS`). The script strips these custom coordinate systems from the target array design after replication, reassigning all generated elements to the `"Global"` coordinate system.
