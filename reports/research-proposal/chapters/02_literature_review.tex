...
\begin{itemize}
    \item \emph{Single-bounce:} $|a|^2/2$ corresponds to odd-bounce scattering, typically arising from spheres, flat plates, or trihedral reflectors ($\vec S_{\text{odd}}$).
    In an automotive context, this mechanism dominates returns from flat surfaces, such as walls or the rear of a vehicle.
    \item \emph{Double-bounce:} $|b|^2/2$ corresponds to even-bounce scattering derived from dihedral structures ($\vec S_{\text{even}}$).
    ...
    \item \emph{Cross-polar:} $|c|^2/2$ represents the cross-polarization energy induced by dihedrals or dipoles rotated by $\ang{45}$ around the radar line-of-sight ($\vec S_{\text{even}}(\ang{45})$).
...
\paragraph{Bridge to research gaps and proposed methodology.}
