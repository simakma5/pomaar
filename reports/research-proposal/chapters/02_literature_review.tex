...
In this doctoral work, the horizontal-vertical ($H/V$) linear polarization basis is selected as the primary physical measurement baseline.
Crucially, acquiring the full complex monostatic backscattering matrix $\vec S$ in the $H/V$ basis incurs no loss of generality: any alternative polarization basis $\vec S'$ can be synthesized offline through a unitary basis transformation,
\begin{equation}
...
Canonical targets possess well-defined, deterministic geometry, yielding exact analytical forms for their Sinclair scattering matrices $\vec S$ in the linear polarization basis.
...
For a perfectly conducting sphere, flat specular plate, or trihedral corner reflector at broadside incidence, the co-polarized returns are identical in amplitude and phase ($S_{\mathrm{HH}} = S_{\mathrm{VV}}$), while cross-polarized coupling is zero ($S_{\mathrm{HV}} = S_{\mathrm{VH}} = 0$).
...
A dihedral corner reflector induces two sequential reflections before returning energy to the radar.%
    \footnote{The same mechanism described in this section occurs for any even number of reflections, such as a four-bounce reflection off a wall-corner-wall configuration. However, for automotive radar, the dihedral is the most common and practical realization of this mechanism, hence the focus on this configuration.}
Reflecting off two metallic surfaces introduces a $\pi$ phase reversal between the electric field component parallel to the vertex line and the component perpendicular to it.
When a dihedral is aligned with the antenna polarization axis ($\theta = \ang{0}$), it yields equal co-polar amplitudes with opposite phase ($S_{\mathrm{HH}} = -S_{\mathrm{VV}}$) and zero cross-polarization ($S_{\mathrm{HV}} = S_{\mathrm{VH}} = 0$):
\begin{equation}
...
\end{equation}
In this configuration, the co-polarized channels are completely suppressed ($S_{\mathrm{HH}} = S_{\mathrm{VV}} = 0$), while all backscattered energy is transferred into the cross-polarized channels ($S_{\mathrm{HV}} = S_{\mathrm{VH}} = \sigma_{\text{dih}}$).
...
\end{equation}
For a horizontal dipole ($\theta = \ang{0}$), only $S_{\mathrm{HH}}$ is excited, whereas a vertical dipole ($\theta = \ang{90}$) yields pure $S_{\mathrm{VV}}$ backscatter.
...
\begin{align}
    ...
    \vec k_P = \frac{1}{\sqrt{2}} \begin{bmatrix}
        ...
        2S_{12}
    \end{bmatrix}.
\end{align}~\parencite{cloudeReviewTargetDecomposition1996}
...
Mathematically, they represent single-look diagonal power ratios and cross-channel correlations derived from the outer product of the lexicographic target vector $\vec k_L$ or transformed from the target coherency matrix $\vec T$ from \cref{eq:pauli-coherency-matrix}.%
    \footnote{Taking the outer product of the lexicographic vector forms a \emph{target covariance matrix} $\vec C$, which is mathematically equivalent to the Pauli coherency matrix $\vec T$ under a unitary transformation~\parencite{cloudeReviewTargetDecomposition1996}.}
...
\begin{align}
    ...
    Q_{xy} = \frac{\sum_{i=1}^N |S_{xy}^i|^2}{\sum_{i=1}^N \Big( |S_{\mathrm{HH}}^i|^2 + |S_{\mathrm{HV}}^i|^2 + |S_{\mathrm{VH}}^i|^2 + |S_{\mathrm{VV}}^i|^2 \Big)},
\end{align}
