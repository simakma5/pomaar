...
This is reflected again in \cref{fig:poincare-sphere}: fully polarized waves lie on the surface ($\text{DOP} = 1$), while partially polarized states lie within the volume ($\text{DOP} < 1$).
...
\begin{figure}[t]
    \centering
    \input{figures/poincare_sphere.tikz}
    \caption{
...
\begin{figure}[htbp]
    \centering
    \input{figures/h_alpha_plane.tikz}
...
\begin{itemize}
    \item \textit{Volume (canopy) scattering:} $\vec C_{\mathrm{v}}$ models volumetric clutter (such as tree foliage or vegetation canopy) as an ensemble of randomly oriented, thin cylindrical dipoles.
    Assuming a uniform orientation distribution over $[-\uppi, \uppi]$, this yields an isotropic volume covariance matrix.
...
\begin{itemize}
    \item \textit{Helix scattering component:} $\vec C_{\mathrm{h}}$ accounts for non-reflection-symmetric scattering mechanisms, such as quarter-wave phase shifts, chiral interactions, and circular depolarization generated by complex metallic geometries viewed at oblique angles.
    Drawing directly from the helix canonical scatterer defined in \cref{sec:canonical-scatterers}, the helix power coefficient $f_{\mathrm{h}}$ is extracted directly from the cross-channel correlation.
    \item \textit{Orientation-adaptive volume scattering:} Yamaguchi $\vec C_{\mathrm{v}}$ replaces the rigid uniform dipole assumption with orientation \glspl{pdf}.
    By evaluating the co-polar power ratio $\langle |S_\HH|^2 \rangle / \langle |S_\VV|^2 \rangle$, the volume matrix adapts to horizontal dipole dominance (e.g. horizontal vehicle structures), vertical dipole dominance (e.g. roadside posts and standing crops), or uniform random dipoles, preventing the overestimation of volume clutter over oriented man-made targets.
\end{itemize}
...
\begin{equation}
    ...
    \end{bmatrix},
\end{equation}
where the plus and minus signs denote left-handed and right-handed helix covariance matrices, respectively.
By absorbing non-reflection-symmetric cross-channel energy into $f_{\mathrm{h}}$ and refining the volume model $\vec C_{\mathrm{v}}$, the Yamaguchi decomposition eliminates negative power artifacts and provides robust physical parameter extraction in complex urban scenes.
