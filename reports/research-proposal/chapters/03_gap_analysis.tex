...
\begin{custombox}{Research questions}
    \begin{enumerate}
        \item \textit{Can the integration of a high-resolution $12 \times 16$ \gls{mimo} architecture with Doppler-resolved polarimetric processing significantly enhance the classification accuracy of dynamic \glspl{vru}?}
        \item \textit{Which polarimetric decomposition methods are most robust to the non-stationary, grazing-incidence, and multipath-dominated conditions of automotive radar, and how can they be adapted to exploit Doppler-resolved features for improved \gls{vru} classification?}
        \item \textit{What are the fundamental lower bounds on antenna requirements, as derived from the sensitivity of polarimetric decomposition algorithms?}
        \item \textit{How can \gls{mimo} topologies be optimized specifically for polarimetric fidelity to mitigate the detrimental effects of phase centre displacement and cross-polar coupling?}
    \end{enumerate}
\end{custombox}
