...
\begin{quote}
    \enquote{The angle for which the sum of the squared absolute values of the scattering matrix is maximum is computed\dots
    This procedure effectively selects the dominant target within a range-Doppler cell, thus removing the angular dimension from the processed measurement data.}
\end{quote}
...
\begin{custombox}[orange]{To-do: Olexander's comment}
    I would not dive into this subject, its is multiple PhDs on its own.
    I would rather model scatterer as a black box with some polarization properties and see which antennas are needed to reconstruct them from measured \gls{mimo} data.
\end{custombox}
...
\begin{custombox}[orange]{To-do: Olexander's comment}
    \begin{itemize}
        \item Decomposition is an advanced step.
        I would start with simple (power only) measurement technique to reconstruct polarimetric properties of the scatterer - similar to what Mate is doing now, but Mate has dual pol antennas in each measurement point.
        How to do the same power-only with receive antennas displaced from each other?
        how to calibrate out phase shift due to the antenna displacement?
        \item The bloody question here: how to do averaging (mathematical operation $\langle a\rangle$)?
        Averaging in time -- but car is moving.
        Averaging over multiple image cells -- like in \gls{sar} -- but we do not have so many cells\dots
    \end{itemize}
\end{custombox}
...
Although classical one-dimensional theory dictates a length slightly less than $\lambda_{\mathrm{g}}/4$, localized dielectric loading and fringing capacitance can reduce the actual physical length to merely \qtyrange{35}{40}{\percent} of the theoretical quarter-wavelength. Shortening the stub rotates the impedance locus downwards in the capacitive direction on the Smith chart.
